...
async def quiz_index(request: Request):
    """
    127.0.0.1:8000 にアクセスされたときの入口。

    - 未ログイン -> /login へ
    - 本登録ユーザーで display_name 未設定 -> /login へ（★要件どおり）
    - それ以外 -> home.html を表示
    """
    ...
    # ★ 本登録ユーザーで display_name 未設定なら /login
...
async def home(request: Request):
    ...
    # ★ ここも /auth/login ではなく /login に修正
    if not user:
        return RedirectResponse("/login", status_code=303)

    # ★ 本登録ユーザーで display_name 未設定なら /name に飛ばしたい場合はここで制御
    if not getattr(user, "is_guest", False) and not getattr(user, "display_name", None):
        # ※「名前入力ページ」が /name だと想定
...
@app.get("/bgm-player")
def bgm_player(request: Request):
    return templates.TemplateResponse("bgm_player.html", {"request": request})
